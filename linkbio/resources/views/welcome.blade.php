...
@section('title', config('app.name').' — Satu Link untuk Semua Kontenmu')
@section('description', 'Buat halaman link-in-bio profesional dalam hitungan menit. Gabungkan Instagram, TikTok, YouTube, WhatsApp, dan semua tautanmu dalam satu link.')
        ...
        <div class="max-w-7xl mx-auto px-5 sm:px-8 grid lg:grid-cols-2 gap-16 items-center">
            <div class="animate-fade-up">
                ...
                <div class="mt-8 flex flex-col sm:flex-row gap-4">
                    <a href="{{ route('register') }}" class="px-7 py-3.5 rounded-full bg-brand-600 text-white font-semibold text-center shadow-soft hover:bg-brand-700 hover:-translate-y-0.5 transition-all">
                ...
                <div class="relative w-[280px] sm:w-[320px]">
                    <div class="rounded-[2.5rem] border-8 border-ink-900 bg-ink-900 shadow-2xl overflow-hidden">
                        ...
                        <div class="bg-white p-4 space-y-3">
                            @foreach (['📸 Instagram', '🎵 TikTok', '▶️ YouTube', '💬 WhatsApp', '🐙 GitHub'] as $item)
    ...
    <section class="py-24">
        <div class="max-w-5xl mx-auto px-5 sm:px-8">
            <div class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-brand-600 to-indigo-700 px-8 py-16 sm:px-16 text-center shadow-2xl">
                ...
                <a href="{{ route('register') }}" class="mt-8 inline-block px-8 py-4 rounded-full bg-white text-brand-700 font-bold shadow-lg hover:-translate-y-0.5 hover:shadow-xl transition-all">
            ...
            <div class="mt-12 space-y-4" x-data="{ open: 0 }">
                ...
                @foreach ($faqs as $i => $faq)
                    <div class="rounded-2xl border border-ink-100 bg-white overflow-hidden">
                        <button @click="open = open === {{ $i }} ? -1 : {{ $i }}" class="w-full flex items-center justify-between gap-4 px-6 py-5 text-left font-semibold text-ink-900">
                            {{ $faq['q'] }}
                            <span x-text="open === {{ $i }} ? '−' : '+'" class="text-brand-600 text-xl flex-shrink-0"></span>
                        </button>
                        <div x-show="open === {{ $i }}" x-collapse x-cloak class="px-6 pb-5 text-sm text-ink-600 leading-relaxed">
                            {{ $faq['a'] }}
                        </div>
                    </div>
                @endforeach
