...
@section('content')
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
        @php
            ...
            $cards = [
                ['label' => 'Total Link', 'value' => number_format($totalLinks), 'icon' => '🔗', 'color' => 'from-brand-500 to-indigo-600'],
                ['label' => 'Link Aktif', 'value' => number_format($activeLinks), 'icon' => '✅', 'color' => 'from-emerald-500 to-teal-600'],
                ['label' => 'Total Klik Link', 'value' => number_format($totalClicks), 'icon' => '🖱️', 'color' => 'from-orange-400 to-pink-500'],
            ];
        @endphp
    ...
    <div class="mt-8 grid lg:grid-cols-3 gap-6">
        ...
        <div class="bg-gradient-to-br from-brand-600 to-indigo-700 rounded-2xl p-6 text-white shadow-soft flex flex-col">
            <div class="flex items-center gap-3">
                ...
                <div class="min-w-0">
                    <p class="font-bold truncate">{{ $profile->display_name ?: $user->name }}</p>
                    <p class="text-brand-100 text-sm truncate">/{{ $user->username }}</p>
                </div>
            </div>
            ...
            <div class="mt-auto pt-6 space-y-2">
                <a href="{{ $user->publicUrl() }}" target="_blank" class="block w-full py-2.5 rounded-full bg-white text-brand-700 text-center text-sm font-bold hover:bg-brand-50 transition">
