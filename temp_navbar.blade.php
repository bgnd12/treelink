       ...
       <div class="flex items-center justify-between transition-all duration-300 ease-out"
           :class="scrolled ? 'py-3' : 'py-5'">
              ...
              <div class="hidden md:flex items-center gap-8 text-sm font-medium transition-colors duration-300"
                ...
                <a href="{{ route('home') }}" class="hover:text-brand-600 transition">{{ __('Home') }}</a>
                <a href="{{ route('home') }}#features" class="hover:text-brand-600 transition">{{ __('Features') }}</a>
                <a href="{{ route('home') }}#how-it-works" class="hover:text-brand-600 transition">{{ __('About') }}</a>
                <a href="{{ route('home') }}#contact" class="hover:text-brand-600 transition">{{ __('Contact') }}</a>
            </div>
            ...
            <div class="hidden md:flex items-center gap-3">
                ...
                @else
                          <a href="{{ route('login') }}" class="text-sm font-semibold transition-colors duration-300 hover:text-brand-600"
                              :style="scrolled ? 'color:#334155' : 'color:#ffffff'">{{ __('Log in') }}</a>
        ...
        <div x-show="open" x-cloak x-transition class="md:hidden pb-4 flex flex-col gap-3 text-sm font-medium text-ink-700">
            <a href="{{ route('home') }}" class="py-1">{{ __('Home') }}</a>
            <a href="{{ route('home') }}#features" class="py-1">{{ __('Features') }}</a>
            <a href="{{ route('home') }}#how-it-works" class="py-1">{{ __('About') }}</a>
            <a href="{{ route('home') }}#contact" class="py-1">{{ __('Contact') }}</a>
            <div class="flex gap-3 pt-2">
                ...
                @auth
                    <a href="{{ auth()->user()->is_admin ? route('admin.index') : route('dashboard.index') }}" class="px-4 py-2 rounded-full bg-ink-900 text-white text-center flex-1">{{ __('Dashboard') }}</a>
                @else
                    <a href="{{ route('login') }}" class="px-4 py-2 rounded-full border border-ink-200 text-center flex-1">{{ __('Log in') }}</a>
                    <a href="{{ route('register') }}" class="px-4 py-2 rounded-full bg-brand-600 text-white text-center flex-1">{{ __('Get Started') }}</a>
                @endauth
            </div>
        </div>
    </div>
